...
- **Reconnexion silencieuse.** Pour éviter de redemander un clic "Se connecter" à chaque ouverture du formulaire, `GoogleAuthService` mémorise dans localStorage (`google_auth_deja_consenti`, un simple booléen — jamais le token) le fait que l'utilisateur a déjà accordé le scope une fois sur cet appareil, puis tente un `requestAccessToken({prompt: ''})` sans popup (`tenterReconnexionSilencieuse()`, appelé au montage d'`AjoutLieuComponent`). Ça ne fonctionne que si la session Google du navigateur est toujours active ; sinon l'échec est silencieux et l'écran de connexion habituel reprend la main — pas de garantie que ça évite systématiquement une reconnexion (dépend de la session/cookies Google, pas d'un vrai refresh token).
